...
| --- | --- | --- | --- | --- |
| Service API (`usermgmt.v1.UserService` over gRPC) | `localhost:10102` | Internal mesh only | Full CRUD | [Proto](api/proto/README.md), [user.proto](api/proto/usermgmt/v1/user.proto) |
...
Bind the three listeners on separate ports so they can be firewalled independently. Do not expose the Service API or `/internal` publicly.
...
| `SERVICE_API_PORT` | `10102` | Host / Service API port |
...
| `SERVICE_API_ADDR` | `:10102` | Service API listen address |
...
```bash
make smoke              # public REST + internal REST + Service API
make smoke-public
make smoke-internal
make smoke-service
```
...
Local ports come from `.env` (`10100` public, `10101` internal, `10102` Service API). Copy `id` from the create response into the get calls.
...
View a user (Service API). Use [grpcurl](https://github.com/fullstorydev/grpcurl); the server exposes reflection:
...
Regenerate Service API stubs after proto changes:
